#!/bin/bash
# Superspec YAML Schema Validator — validates state.yaml structure
# Usage: superspec-yaml-validate.sh <change-name>
...
if [ ! -d "$CHANGE_DIR" ] && [ -d ".superspec/missions/Deliver/$CHANGE" ]; then
...
# --- Required fields ---
REQUIRED_FIELDS="workflow phase blueprint_doc plan build_mode isolation refine_mode refine_result verified_at archived"
...
validate_enum "phase"         "$phase"          "Vision Blueprint Forge Refine Deliver"
...
if [ -n "$blueprint_doc" ] && [ "$blueprint_doc" != "null" ]; then
  if [ ! -f "$blueprint_doc" ]; then
    fail "blueprint_doc='$blueprint_doc' does not exist on disk"
  fi
fi
