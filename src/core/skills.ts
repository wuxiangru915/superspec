...
description: Run the {skillName} Superspec workflow
---
`;
...
async function copySuperspecSkillsForPlatform(
  baseDir: string,
  platform: Platform,
  overwrite: boolean,
  ...
  for (const skillRelPath of manifest.skills) {
    ...
    const isEmbedded = skillRelPath.startsWith('embedded/');
    ...
    let destRelPath = skillRelPath;
    if (isEmbedded) {
      // Flatten embedded skills: embedded/openspec/foo.md -> openspec-foo.md
      // Or if it's a specific folder structure, handle it.
      // Current embedded structure: embedded/openspec/explore.md
      // We want it to land in: skills/openspec-explore.md (or similar)
      // Actually, let's keep it simple: strip 'embedded/' and 'openspec/' and just use the filename with prefix
      const parts = skillRelPath.split('/');
      const category = parts[1].replace('-zh', ''); // openspec or superpowers
      const filename = parts[parts.length - 1];
      destRelPath = `${category}-${filename}`;
    }
...
async function createOpenCodeCommands(
  ...
  for (const skillPath of skillPaths) {
    ...
    const skillSourcePath = path.join(assetsDir, 'skills', skillPath);
...
Equivalent Superspec skill: \`${skillName}\`
...
/**
 * Copy Superspec rule files to a platform's rules directory.
 * Formats:
 ...
 */
async function copySuperspecRulesForPlatform(
  baseDir: string,
  platform: Platform,
  overwrite: boolean,
  scope: InstallScope = 'project',
...
function computeRuleDestPath(
  ...
  if (rulesFormat === 'copilot') {
    // GitHub Copilot: superspec-phase-guard.md → superspec-phase-guard.instructions.md
...
/**
 * Install Superspec hooks for platforms that support them.
 ...
 */
async function installSuperspecHooksForPlatform(
  baseDir: string,
  platform: Platform,
  scope: InstallScope = 'project',
 ...
 * Writes to .github/hooks/superspec-guard.json with preToolUse hooks config.
 */
async function installCopilotHooks(
  ...
  const hookFilePath = path.join(hooksDir, 'superspec-guard.json');
...
/**
 * Kiro format:
 * Writes to .kiro/hooks/superspec-phase-guard.kiro.hook as a JSON file.
 */
...
async function createWorkingDirs(projectPath: string): Promise<void> {
  const dirs = [
    path.join(projectPath, '.superspec'),
    path.join(projectPath, '.superspec', 'missions'),
    path.join(projectPath, '.superspec', 'missions', 'archive'),
    path.join(projectPath, '.superspec', 'specs'),
  ];
  ...
  const configPath = path.join(projectPath, '.superspec', 'config.yaml');
